...
log_regression = LogisticRegression(max_iter=2000, random_state=42, solver='saga')
...
"""Logistic performed better in the dataset as the accuracy on test set is 94.3 and the accuracy on train set is 94.27"""
...
# Define the parameter grid with more focused ranges and distributions
param_grid_dt = {
    'max_depth': randint(5, 50),              # Using randint for integer values
    'min_samples_split': randint(2, 20),      # Reduced upper range to prevent overfitting
    'min_samples_leaf': randint(1, 10),       # Reduced upper range to prevent overfitting
    'max_features': ['sqrt', 'log2', None],   # Removed fractional values as they often lead to suboptimal results
    'criterion': ['gini', 'entropy'],         # Removed log_loss as it's less stable
    'splitter': ['best'],                     # 'best' typically performs better than 'random'
    'class_weight': ['balanced'],             # Using balanced weights to handle any class imbalance
    'min_impurity_decrease': uniform(0, 0.1), # Added to control tree growth
    'ccp_alpha': uniform(0, 0.05)             # Added cost-complexity pruning parameter
}

# Initialize the decision tree classifier
dt_model = DecisionTreeClassifier(random_state=42)

# Initialize Stratified K-Folds with fewer splits to prevent overfitting
cv = StratifiedKFold(n_splits=5, shuffle=True, random_state=42)

# Initialize RandomizedSearchCV with more iterations
random_search_dt = RandomizedSearchCV(estimator=dt_model,param_distributions=param_grid_dt,n_iter=100,cv=cv,scoring=['accuracy', 'f1'],refit='accuracy',random_state=42,n_jobs=-1,verbose=2,return_train_score=True        )
...
# Define the parameter grid (simplified for a quick try)
param_grid_lr = {
    'C': np.logspace(-4, 4, 10),       # Fewer C values for faster tuning
    'penalty': ['l1', 'l2'],             # Try l1 and l2 regularization
    'solver': ['liblinear', 'saga'],     # Common solvers
    'max_iter': [1000, 2000]          # Iteration limits
}

# Initialize Stratified K-Folds
cv = StratifiedKFold(n_splits=5, shuffle=True, random_state=42)  # Reduced folds for speed

# Initialize RandomizedSearchCV (fewer iterations)
random_search_lr = RandomizedSearchCV(estimator=lr_model,param_distributions=param_grid_lr,n_iter=20,
                                     cv=cv,
                                     scoring='accuracy',
                                     random_state=42,
                                     n_jobs=-1,
...
log_regression = LogisticRegression(random_state=42, max_iter=500)

# Initialize SelectFromModel
sfm_log_regression = SelectFromModel(log_regression)

# Fit SFM into training data
sfm_log_regression.fit(X_train_scaled, y_train)
...
best_params_dt = {'ccp_alpha': 0.019654886233338022, 'class_weight': 'balanced', 'criterion': 'entropy', 'max_depth': 12, 'max_features': None, 'min_impurity_decrease': 0.04343943655104287, 'min_samples_leaf': 5, 'min_samples_split': 2, 'splitter': 'best'}
...
lr_selected_features = ['PM10', 'NO2', 'CO']
